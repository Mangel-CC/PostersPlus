...
def get_cached_tmdb_metadata(cache_key: str) -> dict | None:
    try:
        ...
        age_days = (time.time() - cached_at) / 86400
        # Airing shows get a shorter TTL — last_episode_to_air moves weekly and
        # the new-episode sash depends on it being fresh.
...
def delete_cached_release_status(cache_key: str) -> None:
    ...
    try:
        with _db_lock:
            get_db().execute(
                "DELETE FROM release_status_cache WHERE cache_key = ?", (cache_key,)
            )
            get_db().commit()
