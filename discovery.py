...
def _evaluate_slot(slot: str, meta: DiscoveryMeta) -> str | None:
    ...
    if slot == "wins":
        return " • ".join(meta.award_wins) if meta.award_wins else None

    if slot == "pic_noms":
        # Only Best Picture nominations — identified by the stored label
        pic = [n for n in meta.award_noms if "Best Picture" in n]
        return " • ".join(pic) if pic else None

    if slot == "emmy_noms":
        # Only Emmy nominations — identified by the stored label
        emmy = [n for n in meta.award_noms if "Emmy" in n]
        return " • ".join(emmy) if emmy else None
...
DEFAULT_SASH_PRIORITY: list[str] = [
    "wins",
    "pic_noms",
    "festival",
    "studio",
    ...
    "cult",
    "foreign",
    "new_release",   # fires on release-date recency OR r/movieleaks digital release
    "metacritic",
    "true_story",
    "structural",
    "emmy_noms",
...
ALL_PRIORITY_SLOTS: list[str] = [
    ...
    "structural",
    "emmy_noms",
    "digital_release",  # legacy alias for new_release — still accepted in sash_priority param
