...
- **Quality badges** - six display modes: Quality Notch (vertical tier-coloured accent pill), Quality + Age Rating (age numeral tinted by 4K/Remux/HDR tier), Badge Row (PNG icons for 4K, 1080p, Remux, Web, DV, HDR10+, HDR10), Combined Text Badge, Age Rating Only, or hidden. A minimum quality threshold (`badge_min_score`) can suppress the badge when stream quality falls below a configurable bar. Sourced from an AIOStreams integration, Torrentio or Comet and fetched in the background on first request. When the source returns nothing (common for anime) — or no source is configured — a release-based fallback (`QUALITY_RELEASE_FALLBACK`, default on) infers a conservative badge: aired TV/anime → Web 1080p, digital movies → Web 1080p, physical → 1080p. Plex and Jellyfin will use your local files' actual quality for completely accurate data.

- **New-episode sash (TV & anime)** - series whose latest episode aired recently (default 7 days, `EPISODE_SASH_MAX_AGE_DAYS`) show an episode-availability sash like `S2E5`. The label template is configurable (`EPISODE_SASH_FORMAT` / `?episode_format=` — e.g. `T{s}E{e}` renders "T2E5", `{s}x{e}` renders "2x5"). Data comes from TMDB's `last_episode_to_air`, so it costs no extra API call. The slot is called `new_episode` in `sash_priority`.

- **Anime detection & style** - titles with the Animation genre and Japanese origin get a dedicated sakura-pink sash palette (new-episode and "Anime" language sashes), and MyAnimeList is boosted to a configurable share of the weighted score (`ANIME_MAL_WEIGHT`, default 0.4) whenever MAL has a rating for the title.
